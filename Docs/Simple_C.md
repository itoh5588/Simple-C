...
Simple C は独立した IR を持たない構成のため、移植境界は `ccgen.py` と `cclib/runtime.asm` に集中している。フロントエンド（`ccpre.py` / `cclex.py` / `ccparse.py`）は CPU 非依存で、触る必要はない。

### 移植で触るファイル

| ファイル | 移植時の作業 |
|---|---|
| `ccpre.py` | 触らない（CPU 非依存） |
| `cclex.py` | 触らない（CPU 非依存） |
| `ccparse.py` | 触らない。ただし `asm(".ADDR ...")` のようなトップレベル asm を使うなら、配置制御の仕様を新 ISA 用に決める |
| `ccgen.py` | **本丸**。命令ニーモニック・レジスタ・即値幅・呼び出し規約・peephole を新 ISA 用に書き直す |
| `cclib/runtime.asm` | エントリポイント・スタック初期化・終了処理を新 ISA 用に書き直す |
| `asm.py` / `emu.py` | 新 ISA を扱うなら入れ替えるか、ターゲット既存の toolchain (`as` / `qemu` 等) に差し替え |

### ccgen.py 内の ISA 固有要素

書き直しが必要になる箇所:

- **レジスタ名**: `R0`-`R9`、`SP`、frame pointer (`R7`)
- **命令ニーモニック**: `MOV` / `MOVI` / `LDD` / `STD` / `PUSH` / `POP` / `ADDI` / `SUBI` / `MULI` / `DIVI` / `MODI` / `ANDI` / `ORI` / `XORI` / `SBT` / `SBTI` / `JPI` / `CALLI` / `RET` 等
- **即値幅**: 20bit。定数畳み込みの境界判定に使っている
- **エンディアン**: big-endian 32bit `.DWORD`。`int[]` グローバル初期化子のバイト並びに影響
- **呼び出し規約**: スタック引数、戻り値 `R0`、OS ABI 用 `R8`/`R9`
- **peephole パターン**: 隣接 `PUSH`/`POP` 同レジスタ、自己 `MOV`、`JPI` 直後ラベル — すべて命令名前提
- **比較-分岐合成**: `SBT` / `SBTI` のフラグ動作に依存
- **シフト**: `<<` を `MULI 2^n`、`>>` を `DIVI 2^n` で実装している箇所は、新 ISA に shift 命令があれば置き換える

### 段階的移植手順

最小から積み上げる。各段で `samples/*.c` の対応する範囲だけ通せばよい。

1. **`int main() { return 0; }` が動く最小 codegen**
   - 関数プロローグ・エピローグ、定数ロード、`RET` 相当だけ
   - `cclib/runtime.asm` のエントリも同時に書く
2. **算術・代入・local 変数**
   - frame pointer / SP 操作、二項演算、複合代入
3. **関数呼び出し・引数**
   - 呼び出し規約を新 ISA に合わせる（レジスタ引数 ABI なら大きな書き直しになる）
4. **グローバル変数・配列・ポインタ**
   - 配置ディレクティブ（書籍 ISA でいう `.ADDR` / `.DWORD` / `.BYTE`）を新 ISA 用に決める
5. **制御構造（`if` / `while` / `for` / `&&` / `||`）**
   - 比較-分岐合成と短絡評価を新 ISA の条件分岐命令に合わせて書く
6. **inline asm と固定アドレス配置**
   - `asm("...")` / `asm(".ADDR ...")` の通し方を新 ISA 用に整える
   - 必要なら最後に peephole を書き直す

### テスト戦略

`samples/*.c` をリグレッションスイートとして使う。各サンプルは対応する機能サブセットに対応しているので、上記の段階ごとに「ここまでで通るべき samples」を切り出せる。OS 動作確認 (`tools/os_run.py`) は書籍 CPU 固有なので、移植先では別途用意する。

### 移植先候補

- **RISC-V (RV32I)**: 書籍 CPU から最も近い。レジスタ 32 本、即値 12bit / 20bit、エンディアンは little だが、命令体系は素直
- **LC-3 / MIPS**: 教育用 ISA、エミュレータが豊富。学習目的にはこちらが向く
- **WASM**: スタックマシンなので codegen は更にシンプルになる。ブラウザで動くデモを作りやすい
- **8080 / Z80 / 6502**: レトロ系。レジスタ数・即値幅が厳しく、frame pointer 戦略から見直す必要がある。Simple C の現生成戦略（local 変数は SP 相対）はそのままだと厳しい
- **x86 / ARM**: 実用的だが、命令体系・呼び出し規約の規模が大きく、Simple C の「素直さ」が損なわれやすい
